...
private:
    ...
    QVBoxLayout* initRepoLayout;
    QFormLayout* initRepoFormLayout;
    ...
    void handleRepositoryInitialization();
};
...
void VersionControlGUI::handleRegister() {
    ...
    try {
        if (controller->registerUser(newUsername, newPassword)) {
            ...
            if (controller->login(newUsername, newPassword)) {
...
void VersionControlGUI::createUserRepositoriesPage() {
    ...
    try {
        vector<QString> userRepos = controller->listUserRepos();
        for (const auto &repo : userRepos) {
            repoListWidget->addItem(repo);
        }
    } catch (const std::exception &e) {
...
void VersionControlGUI::handleCheckRepository() {
    ...
    try {
        ...
        if (controller->repoExists(workingDirectory)) {
            // Display a success message or handle further actions
...
void VersionControlGUI::handleCommitChanges() {
    ...
    try {
        ...
        if (ok && !commitMessage.isEmpty()) {
            ...
            controller->commitChanges(commitMessage);
